...
- **多屏覆盖**：单块屏幕的窗口操作失败时继续处理其他屏幕，并记录失败操作；只在所有休息窗口均失焦时请求一次焦点，避免多屏轮流抢焦点。
- **倒计时初始化**：先订阅状态再读取快照，避免迟到的旧快照覆盖最新倒计时；加载时不再短暂显示误导性的 `00:00`。

### 性能与体验
- **独立休息入口**：每屏休息窗口直接加载倒计时组件，使用不透明深色窗口，减少主应用界面与样式加载和透明窗口合成开销。
- **减少重复操作**：稳定状态不重复调整位置、尺寸或切换全屏；移除每屏重复的状态事件，沿用统一广播。
- **减少动态效果**：休息页遵循系统“减少动态效果”设置，关闭呼吸动画和进度过渡。
- **休息页交互保护**：独立入口保留拖放导航与右键菜单保护。
...
### 新增
- **原型先行的 UI 资产**：将已确认的页面原型、二级交互规范、实现截图、浏览器验收与 Tauri 原生壳验证统一纳入 `designs/work-review-ui-refresh/`，后续界面迭代可以继续通过 Git diff 对照原型与生产实现。
- **时间线 OCR 导出选择**：导出 JSON 前使用应用内弹窗明确选择是否包含 OCR；默认不包含敏感文本，确认后再进入系统原生保存窗口。

### 调整
- **全应用界面重做**：概览、护眼、时间线、日报、助手、设置和关于页统一为固定亮色的紧凑工作台，重新梳理信息层级、阅读轴、操作轴、留白和响应式布局。
- **应用外壳统一**：侧边栏、窗口栏、录制状态、语言入口和主滚动区使用同一套低对比边界、品牌蓝激活态与紧凑尺寸规范。
- **二级交互统一**：确认框、危险操作、清理范围、分类编辑、日报设置/预设和小时摘要统一为 12px 亮色弹窗或右侧抽屉，并补齐遮罩、Escape、焦点约束与焦点恢复。
- **关键页面精修**：时间线改为统一记录行与辅助截图预览；日报强化成稿阅读与导出；助手对齐成熟 AI 产品的阅读轴和输入区；设置收敛为单层工作台；护眼与关于页降低视觉噪声。

### 验证
- 全量前端测试 431 项通过，生产构建通过；七个主页面在默认、最小和宽屏三档共 21 个组合通过视觉验收；时间线 OCR 导出已在真实 Tauri/WebKitGTK 桌面壳验证。
